...
def get_args():
    ...
    parser.add_argument('--method', default='SetXor', type=str, help='method name: SetXor/SetXor_IVW/SFM/HLL/FM/CL/LL')
    ...
    parser.add_argument('--setxor_Msize', default=4096, type=int, help='m of SetXor/SetXor_IVW sketch')
    parser.add_argument('--setxor_Wsize', default=32, type=int, help='w of SetXor/SetXor_IVW sketch')
...
if __name__ == "__main__":
    ...
    for r in range(exp_rounds):
        ...
        dataloader = Dataloader(args.dataset, args.intersection, args.difference, args.ratio, seed)
        dict_dataset = dataloader.load_dataset()
        print('{}th exp_rounds dataset generation finished!'.format(r))
        start_time = time.time()
        if args.method == 'SetXor':
            setxor = SetXor(dict_dataset, args.setxor_Msize, args.setxor_Wsize, args.epsilon, args.output, args.random_response, seed)
            setxor.build_sketch()
            result = setxor.estimate_intersection()
            lst_all_results.append(result[0])
            
        if args.method == 'SetXor_IVW':
            setxor = SetXor(dict_dataset, args.setxor_Msize, args.setxor_Wsize, args.epsilon, args.output, args.random_response, seed)
            setxor.build_sketch()
        ...
        if args.method == "SFM":
            sfm = SFM(dict_dataset, args.sfm_Msize, args.sfm_Wsize, args.epsilon, args.merge_method, seed)
            sfm.build_fm_sketch()
            result = sfm.estimation_itersection_cardinality()
            lst_all_results.append(result[0])

        if args.method == "FM":
            fm = FM(dict_dataset, args.fm_Msize, args.fm_Wsize, seed)
            fm.build_sketch()
            result = fm.estimate_intersection()
            lst_all_results.append(result[0])

        if args.method == "HLL":
            hll = HyperLogLog(dict_dataset, args.hll_Msize, seed)
            hll.build_sketch()
            result = hll.estimate_intersection()
            lst_all_results.append(result[0])
            
        if args.method == "CL":
            cl = CL(dict_dataset, args.cl_Msize, args.cl_l, args.epsilon, seed)
            cl.build_sketch()
            result = cl.estimation_intersection_cardinality()
            lst_all_results.append(result[0])
            
        if args.method == "LL":
            ll = LL(dict_dataset, args.ll_a, args.ll_Msize, args.epsilon, seed)
            ll.build_sketch()
