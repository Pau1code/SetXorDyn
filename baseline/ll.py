...
class LL:
        ...
        for user in self.dict_dataset:
            ...
            for i in range(len(user_dict['elements'])):
                ...
                for p in range(repeattimes):
                    ...
                    item_1 = farmhash.hash32withseed(f'{item}', self.seed)
                    ll_sketch.add_id(item_1)
    ...
    def estimation_union_cardinality(self):
        ...
        data_A = self.intersection + self.difference*self.ratio
        data_B = self.intersection + self.difference*(1-self.ratio)
