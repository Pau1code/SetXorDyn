...
def compute_rrmse(m, true_cardi, results):
    sum = 0
    neg_count = 0
    for i in range(m):
        if results[i] < 0:
...
def half_phi_fun(n, w, lamb):
    z = w / 2.0
    for i in range(w-1):
        z -= math.exp(-n * lamb / math.pow(2.0, (i+1))) / 2.0
    z -= math.exp(-n * lamb / math.pow(2.0, (w-1))) / 2.0
    return z / w
